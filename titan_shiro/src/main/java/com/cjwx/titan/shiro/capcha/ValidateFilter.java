...
public class ValidateFilter extends AccessControlFilter{
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {  
        ...
        //3、此时是表单提交，验证验证码是否正确  
        String verifyCode = httpServletRequest.getParameter(jcaptchaParam);
        return CaptchaService.validateResponse(httpServletRequest,verifyCode);
    }  
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {  
        //如果验证码失败了，存储失败key属性  
        request.setAttribute(failureKeyAttribute, "jCaptcha.error");
        RequestDispatcher rd = request.getRequestDispatcher(this.getLoginUrl());
